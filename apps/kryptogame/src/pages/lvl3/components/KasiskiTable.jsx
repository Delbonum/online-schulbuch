...
const KasiskiTable = () => {
  ...
  const getCellStyle = (row, idx) => {
    ...
    if (row === "geheimtext") {
      for (let [start, end] of rotGeheimtext) {
        if (idx === start)
          styles.push("border-l-2", "border-t-2", "border-b-2", "border-red-400", "text-white", "font-bold");
        else if (idx === end)
          styles.push("border-r-2", "border-t-2", "border-b-2", "border-red-400", "text-white", "font-bold");
        else if (idx > start && idx < end)
          styles.push("border-t-2", "border-b-2", "border-red-400", "text-white", "font-bold");
      }
    }
  ...
  const renderRow = (label, data, rowKey) => (
    <tr>
      ...
      {data.map((char, idx) => (
        <td key={idx} className={`w-6 h-6 text-center ${getCellStyle(rowKey, idx)}`}>
          {char}
        </td>
      ))}
    </tr>
